...
st.markdown("""
    ...
    <div style="font-size:40px; position:fixed; top:50px; right:20px; opacity:0.08;">🤖</div>
    <div style="font-size:40px; position:fixed; bottom:100px; left:30px; opacity:0.08;">😃</div>
    <div style="font-size:40px; position:fixed; bottom:150px; right:100px; opacity:0.08;">💬</div>
...
if menu == "🏠 Beranda":
    st.markdown("""
        ...
        <h1 style='text-align:center; color:#00ffc3;'>Selamat Datang di Deteksi Emosi AI</h1>
        <marquee behavior="scroll" direction="left" scrollamount="6" style="color:#00ffc3;">
            Ayo deteksi perasaan kamu! 😁 😢 😠 😍 💭
        </marquee>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
        ...
        💡 Cocok untuk analisis sosial media, feedback pelanggan, dan tugas AI kuliah!
    """)
...
# ========== DETEKSI EMOSI ========== #
    ...
    if st.button("🚀 Deteksi Sekarang"):
        if user_input.strip():
            loading = st.empty()
            for _ in range(3):
                loading.markdown("<h2 style='text-align: center;'>🔍💭😃🤖</h2>", unsafe_allow_html=True)
                time.sleep(0.3)
                loading.markdown("<h2 style='text-align: center;'>🤖💭😠🔎</h2>", unsafe_allow_html=True)
                time.sleep(0.3)
            loading.empty()
